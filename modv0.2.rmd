...
##creating the season stats for each
...
```{r}
###predicting the score for the matches
...
### creating the number of events

scorepred2 <- goalsfor %>% 
                                  mutate(val100 = round(value *1000,0))
...
goals16h <- season16 %>% filter(minutes > 30) %>%
                          select(was_home, team_h_score, goals_scored, assists, playername) %>% 
                            filter(was_home == "True") 

colnames(goals16h)[2] <- "score"

goals16a <- season16 %>% filter(minutes > 30) %>%
           select(was_home, team_a_score, goals_scored, assists, playername) %>% 
...
play_goal <- x2 %>% filter(fixt < 41) %>%
                        left_join(play19_team, by = "team") %>%
                            group_by(playername, fixt, score) %>%
                                summarise(n = n())
...
goals1 <- uncount(play_goal2, val)
                                            


player_score <- goals1 %>% select(playername, fixt, score, gs) %>%
                            group_by(fixt, playername) %>%
                                    mutate(sim = 1:n())
                                    


goal_t <- goals1 %>% group_by(playername, fixt) %>%
                              summarise(n = n())
...
scorepred20 <- goalsfor %>% mutate(valr = round(value, 2)) %>%
                                  mutate(val100 = round(valr *100,0))


x5 <- uncount(scorepred20, val100)
...
home <- x5 %>% filter(loc == "home") %>%
                    #  filter(fixt < 11) %>%
                        group_by(fixt) %>%
                            mutate(score=sample(score))
 

colnames(home)[1] <- "Team1"

away <- x5 %>% filter(loc == "away") %>%
                     #   filter(fixt < 11) %>%
                        group_by(fixt) %>%
                            mutate(score=sample(score))
 

colnames(away)[1] <- "Team2"


fixtrath <- home %>% left_join(away, by = "fixt") %>%
                        mutate(win = if_else(score.x > score.y, 1,0)) %>%
                            group_by(fixt, Team1) %>%
                              summarise(n = n(), tot = sum(win)) 

colnames(fixtrath)[2] <- "Team"

fixtrata <- away %>% left_join(home, by = "fixt") %>%
                        mutate(win = if_else(score.x > score.y, 1,0)) %>%
                            group_by(fixt, Team2) %>%
                              summarise(n = n(), tot = sum(win)) 


colnames(fixtrata)[2] <- "Team"


fixtrat_all <- fixtrath %>%
                              bind_rows(fixtrata) %>%
                      mutate(winch = tot/n) %>%
                          group_by(Team) %>%
                              arrange(fixt) %>%
...
liv <- fixtrat_all %>% filter(Team == "Liverpool")


ggplot(liv, aes(x = fixtn, y = winch, col = Team)) + geom_line()
...
scorepred7 <- goalagpred %>% 
                                  mutate(val100 = round(va *1000,0))


x7 <- uncount(scorepred7, val100)



goalag <- x7 %>% group_by(team, fixt) %>%
                         mutate(score=sample(sa)) %>%
                        mutate(sim = 1:n())
...
assists <- goals16 %>% bind_rows(goals17) %>%
                            bind_rows(goals18) %>%
                                left_join(playdat, by = "playername") %>%
                              select(score, assists, xA90) %>%
                                filter(!is.na(xA90))
...
assists$assists <- as.factor(as.numeric(assists$assists))




assists_split <- initial_split(assists, prop = 0.8, strata = assists)



assists_train <- training(assists_split)


assists_test <- testing(assists_split)




assists_for <- rand_forest() %>%
                set_mode("classification") %>%
                set_engine("ranger") %>%
                  fit(assists ~ ., assists_train)
...
assists_predict <- season19 %>% group_by(playername) %>%
                            summarise(min(fixture)) %>%
                            select(playername) %>%
                             slice(rep(1:n(), each = 6)) %>%
                              group_by(playername) %>%
                              mutate(score = 0:5) %>%
                                left_join(playdat, by = "playername") %>%
                                  select(playername, xA90, score) %>%
                                    filter(!is.na(xA90))



x_a <- predict(assists_for, new_data = assists_predict, type = "prob")


assists_predict2 <- assists_predict %>% bind_cols(x) %>%
                                      gather("pred", "value", -playername, -xA90, -score) %>%
                                separate(pred, c("pre", "as"), "_")





apred2 <- assists_predict2 %>% 
                                  mutate(val100 = round(value *10000, 0))


acount <- uncount(apred2, val100)

acount$as <- as.numeric(as.character(acount$as))


acount2 <- acount %>% group_by(playername, score, as) %>%
                                  summarise(n =n())


acount3 <- acount %>% mutate(delta = score - as) %>%
                              filter(delta > -1) %>%
                          group_by(playername, score) %>%
                              summarise(totres = n())


acount4 <- acount2 %>% left_join(acount3, by = c("playername", "score")) %>%
                                  mutate(per = n/totres) %>%
                                     mutate(delta = score - as) %>%
                                  filter(delta > -1) 
...
finscpre <- player_score %>% left_join(play19_team, by = "playername") %>%
                           left_join(goalag, by = c("sim", "fixt", "team")) 
